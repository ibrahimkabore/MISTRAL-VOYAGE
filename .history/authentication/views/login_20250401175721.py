    ...
    def post(self, request):
        try:
            ...
            if not refresh_token:
                return Response({"error": "Le jeton de rafraîchissement est requis."}, 
                               status=status.HTTP_400_BAD_REQUEST)
            
            token = RefreshToken(refresh_token)
            
            # Conversion correcte du timestamp Unix en datetime
            expiry_datetime = datetime.fromtimestamp(token['exp'])
            
            # Ajout explicite à la blacklist
