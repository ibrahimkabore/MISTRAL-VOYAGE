...
class AmadeusAPI:
    ...
    def create_flight_order(self, flight_offers, travelers, contact):
        ...
        try:
            ...
            # S'assurer que contact est une liste
            contacts = [contact] if not isinstance(contact, list) else contact
            
            # Créer un dictionnaire de données au format attendu par l'API
            body = {
                "data": {
                    "type": "flight-order",
            ...
            # Appel à l'API Amadeus sans arguments nommés
            response = self.client.booking.flight_orders.post(body)
