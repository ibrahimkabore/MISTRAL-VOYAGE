...
class AmadeusAPI:
    ...
    def create_flight_order(self, flight_offer, travelers, contact):
        """
        Crée une réservation de vol avec l'API Amadeus
        
        Args:
            flight_offers (list): Liste des offres de vol originale d'Amadeus
            travelers (list): Liste des voyageurs formatée pour Amadeus
            contact (dict): Informations de contact formatées pour Amadeus
            
        Returns:
            dict: Réponse de l'API Amadeus (réservation créée)
        """
        try:
            # Préparer les données pour la requête
            flight_offers = [flight_offer]  # Doit être une liste même avec une seule offre
            
            # Appel à l'API Amadeus
            response = self.client.booking.flight_orders.post(
                flight_offers=flight_offers,
                travelers=travelers,
                contacts=[contact]  # Doit être une liste
            )
